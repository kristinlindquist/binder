...
def compute_precision_recall_f1(
    tp: int, fn: int, fp: int, beta: int = 2, **kwargs
) -> Dict[str, float]:
    if tp + fp + fn == 0:
...
def postprocess_nested_predictions(
    ...
    **kwargs,
    ...
    metrics_by_type = {
        entity_type: {"tp": 0, "fn": 0, "fp": 0}
        for entity_type in entity_type_vocab + ["all"]
    }
    start_metrics_by_type = {
        entity_type: {"tp": 0, "fn": 0, "fp": 0}
        for entity_type in entity_type_vocab + ["all"]
    }
    end_metrics_by_type = {
    ...
    for example_index, example in enumerate(examples):
        ...
        for entity_type, start_char, end_char in zip(
            ...
            example["entity_start_chars"],
            example["entity_end_chars"],
        ):
            entity_type_count["all"] += 1
            entity_type_count[entity_type] += 1
            example_annotations.add(
                Annotation(
                    id=example["id"],
        ...
        for feature_index in feature_indices:
            ...
            token_start_mask = np.array(
                features[feature_index]["token_start_mask"]
            ).astype(bool)
            token_end_mask = np.array(features[feature_index]["token_end_mask"]).astype(
                bool
            )
            ...
            for type_id, start_index, end_index in zip(
                type_ids, start_indexes, end_indexes
            ):
        ...
        for t in metrics_by_type.keys():
            for k, v in compute_tp_fn_fp(
                example_predictions
                if t == "all"
                else set(filter(lambda x: x.entity_type == t, example_predictions)),
                example_annotations
                if t == "all"
                else set(filter(lambda x: x.entity_type == t, example_annotations)),
            ).items():
        ...
        example_gold_starts = set(
            (x.entity_type, x.start_char) for x in example_annotations
        )
        example_pred_starts = set(
            (x.entity_type, x.start_char) for x in example_predictions
        )
        for t in start_metrics_by_type.keys():
            for k, v in compute_tp_fn_fp(
                example_pred_starts
                if t == "all"
                else set(filter(lambda x: x[0] == t, example_pred_starts)),
                example_gold_starts
                if t == "all"
                else set(filter(lambda x: x[0] == t, example_gold_starts)),
            ).items():
                start_metrics_by_type[t][k] += v

        example_gold_ends = set(
            (x.entity_type, x.end_char) for x in example_annotations
        )
        example_pred_ends = set(
            (x.entity_type, x.end_char) for x in example_predictions
        )
        for t in end_metrics_by_type.keys():
            for k, v in compute_tp_fn_fp(
                example_pred_ends
                if t == "all"
                else set(filter(lambda x: x[0] == t, example_pred_ends)),
                example_gold_ends
                if t == "all"
                else set(filter(lambda x: x[0] == t, example_gold_ends)),
            ).items():
                end_metrics_by_type[t][k] += v

    metrics = collections.OrderedDict()
    precisions, recalls = [], []
    sorted_entity_types = ["all"] + sorted(
        entity_type_vocab, key=lambda x: entity_type_count[x], reverse=True
    )
    for x, x_metrics_by_type in {
        "span": metrics_by_type,
        "start": start_metrics_by_type,
        "end": end_metrics_by_type,
    }.items():
        metrics[x] = {}
        for t in sorted_entity_types:
            ...
            f1, precision, recall = (
                metrics_for_t["f1"],
                metrics_for_t["precision"],
                metrics_for_t["recall"],
            )
            if x == "span" and t != "all":
    ...
    for t in sorted_entity_types:
        ...
        for x in metrics:
            f1, precision, recall = (
                metrics[x][t]["f1"],
                metrics[x][t]["precision"],
                metrics[x][t]["recall"],
            )
            logger.info(
                f"F1 = {f1:>6.1%}, Precision = {precision:>6.1%}, Recall = {recall:>6.1%} (for {x})"
            )
    ...
    if output_dir is not None:
        ...
        for pred in all_predictions:
            ...
            example_id_to_predictions[example_id].add(
            ...
            for entity_type, start_char, end_char in zip(
                ...
                gold_ner.add(
                    (
                        ...
                        end_char,
                        entity_type,
                        example["text"][start_char:end_char],
                    )
                )
...
def remove_overlaps(pred_scores: Dict[Annotation, float]) -> Set:
    predictions, starts = [], []
    for pred in sorted(
        pred_scores, key=lambda x: (-pred_scores[x], x.start_char, x.end_char)
    ):
...
def error_analysis(
    annotations: Set[Annotation], predictions: Set[Annotation], entity_types: List[str]
) -> Dict:
    ...
    fp_patterns = "|".join(
        sorted(set([k[0].lower() for k, _ in fp_counter.most_common(30)]))
    )
    ret = collections.OrderedDict(
        {
            "most-common fp patterns": fp_patterns,
            "most-common fp errors": {
                " | ".join(k): v for k, v in fp_counter.most_common(30)
            },
            "most-common fn errors": {
                " | ".join(k): v for k, v in fn_counter.most_common(30)
            },
        }
    )
    for t in entity_types:
        ...
        fp_patterns = "|".join(
            sorted(set([k.lower() for k, _ in fp_counter.most_common(10)]))
        )
        ret.update(
            {
                t: {
                    "most-common fp patterns": fp_patterns,
                    "most-common fp errors": {
                        k: v for k, v in fp_counter.most_common(10)
                    },
                    "most-common fn errors": {
                        k: v for k, v in fn_counter.most_common(10)
                    },
                }
            }
        )
        ...
        for feature_index in feature_indices:
            ...
            for type_id, start_index, end_index in zip(
                ...
                if pred in example_pred_scores:
                    example_pred_scores[pred] = max(
                        example_pred_scores[pred],
                        span_logits[type_id][start_index][end_index],
                    )
                else:
                    example_pred_scores[pred] = span_logits[type_id][start_index][
                        end_index
                    ]
