...
def postprocess_nested_predictions(
    examples,
    features,
    predictions: Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray], # start_logits, end_logits, span_logits, linked_logits
    id_to_type: List[str],
    ...
    **kwargs,
    ...
    if len(predictions) != 4:
        raise ValueError("`predictions` should be a tuple with three elements (start_logits, end_logits, span_logits, linked_logits).")
    all_start_logits, all_end_logits, all_span_logits, linked_logits = predictions
    ...
    for example_index, example in enumerate(examples):
        ...
        for feature_index in feature_indices:
            ...
            linked_logits = linked_logits[feature_index]

            type_ids, start_indexes, end_indexes, linked_entities = (
                token_start_mask[np.newaxis, :, np.newaxis] & token_end_mask[np.newaxis, np.newaxis, :] & span_preds & linked_logits
            ).nonzero()
            ...
            for type_id, start_index, end_index, linked_entity in zip(type_ids, start_indexes, end_indexes, linked_entities):
